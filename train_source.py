...
def get_args():
    parser = argparse.ArgumentParser(description="Source Domain Pre-train")
    # Experiment Name
    ...
    parser.add_argument("--backbone", default="resnet50",
                        help="Which backbone network to use, only resnet18 | resnet50 | resnet101 are supported")
...
class Trainer:
    ...
    def _do_epoch(self):
        ...
        for iter, ((data, class_l), d_idx) in enumerate(self.source_loader):
            ...
            if iter % (len(self.source_loader) // 5) == 0:
                logging.info("iter {}/{} loss: {:.5f}".format(iter, len(self.source_loader), loss.item()))
    ...
    def do_training(self):
        ...
        for self.current_epoch in tqdm(range(self.args.epochs)):
            ...
            logging.info(f'>>> Epoch: {self.current_epoch} Val Acc: {accuracy:.4f} LR: {current_lr:.6f}')
